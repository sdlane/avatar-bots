...
async def test_server(db_conn):
    ...
    await db_conn.execute("DELETE FROM SpiritNexus WHERE guild_id = $1;", TEST_GUILD_ID)
...
async def test_server_multi_guild(db_conn):
    ...
    await db_conn.execute("DELETE FROM SpiritNexus WHERE guild_id IN ($1, $2);", TEST_GUILD_ID, TEST_GUILD_ID_2)
