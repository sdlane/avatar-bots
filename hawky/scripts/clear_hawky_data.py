#!/usr/bin/env python3
"""
Clear all hawky-related data for a specific guild.

This script removes all hawky data including:
- Sent letters
- Hawky tasks
- Aliases
- Characters
...
Usage:
    python clear_hawky_data.py <guild_id>

Example:
    python clear_hawky_data.py 1234567890
"""
import asyncio
import asyncpg
import sys
...
async def clear_hawky_data(conn: asyncpg.Connection, guild_id: int) -> dict:
    """
    Delete all hawky data for a specific guild.
...
async def main():
    """Main function to connect to database and clear hawky data."""
    # Check arguments
    if len(sys.argv) != 2:
    ...
    try:
        print(f"Clearing all hawky data for guild ID: {guild_id}")
        print("=" * 70)
        ...
        if total_rows == 0:
            print("\n  No hawky data found for this guild.")
            print("=" * 70)
            return
        ...
        response = input(f"Are you sure you want to delete ALL hawky data ({total_rows} total rows)? (yes/no): ")
        ...
        results = await clear_hawky_data(conn, guild_id)
        ...
        print()
        print("✓ Successfully cleared all hawky data.")
        print("=" * 70)
