...
class Product:
    ...
    async def upsert(self, conn: asyncpg.Connection):
        ...
        query = """
        INSERT INTO Product (
            item_number, name, macro, product_type, flavor_text, rules_text, skip_export
        )
        ...
        await conn.execute(
            ...
            self.flavor_text, self.rules_text, self.skip_export
        )
    ...
    async def fetch_by_item_number(cls, conn: asyncpg.Connection, item_number: str) -> Optional["Product"]:
        ...
        row = await conn.fetchrow("""
            SELECT id, item_number, name, macro, product_type, flavor_text, rules_text, skip_export
            FROM Product
            WHERE item_number = $1;
        """, item_number)
        if not row:
